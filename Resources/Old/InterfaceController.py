...
class InterfaceController(QRunnable):
    ...
    def run(self):
        ...
        while self.settings.cur_tracking_object_frame_num >= 0:
            try:
                frame_and_result_rect_and_benckmark = self.frame_queue.get(timeout=0.5)
                self.settings.first_frame = frame_and_result_rect_and_benckmark[0][0]
                self.emit_pic(frame_and_result_rect_and_benckmark)
            except Empty:
                pass
        ...
        while not (self.settings.if_end and self.frame_queue.qsize() == 0):
            try:
                frame_and_result_rect_and_benckmark = self.frame_queue.get(timeout=1)
            except Empty:
                continue
